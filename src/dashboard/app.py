...
def _format_dashboard_tables(tables: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    if "risk_limit_breaches" in formatted:
        frame = formatted["risk_limit_breaches"]
        for column in ["threshold", "observed", "comparison_value"]:
            if column in frame.columns:
                frame[column] = frame[column].map(_format_percent)
    ...
    if "rebalance_reason_summary" in formatted:
        ...
        for column in ["total_days", "rebalance_days", "calendar_days", "drift_days", "calendar_and_drift_days"]:
            if column in frame.columns:
                frame[column] = frame[column].map(_format_integer)
        if "rebalance_ratio" in frame.columns:
            frame["rebalance_ratio"] = frame["rebalance_ratio"].map(_format_percent)
    ...
    if "output_inventory" in formatted:
        frame = formatted["output_inventory"]
  ...
  <div class="shell">
    ...
    <div class="grid">
      ...
      <section class="card">
        ...
        {dataframe_to_html_table(risk_limit_checks)}
      </section>
      <section class="card">
      ...
      <section class="card">
        ...
        {dataframe_to_html_table(rebalance_reason_summary)}
      </section>
      <section class="card">
        <h2>Recent Rebalance Reasons</h2>
        {dataframe_to_html_table(rebalance_reason)}
      </section>
      <section class="card">
